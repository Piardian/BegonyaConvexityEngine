...
def load_2021_data_with_warmup(symbol: str, cache_dir: Path) -> pd.DataFrame:
    """2020-01-01'den itibaren veri çekerek 2021 yılı için 200 EMA warm-up sağlar."""
    ticker = TICKER_MAP.get(symbol, symbol)
    cache_file = cache_dir / f"{symbol}_2020_2022.csv"
    ...
    if df.empty or len(df) < 300:
        df = yf.download(ticker, start="2020-01-01", end="2022-01-15", interval="1d", progress=False)
        if df.empty:
            return pd.DataFrame()
...
def run_2021_gold_crypto_backtest():
    ...
    all_2021_trades: List[ConvexityTrade] = []
    symbol_reports: List[ConvexityReport] = []

    for symbol in symbols:
        df = load_2021_data_with_warmup(symbol, cache_dir)
        ...
        # SADECE 2021 yılında açılan işlemler
        trades_2021 = [t for t in full_trades if "2021-01-01" <= t.entry_date <= "2021-12-31"]
        all_2021_trades.extend(trades_2021)

        rep = PerformanceReporter.calculate(trades_2021, name=symbol, initial_capital=initial_capital)
        symbol_reports.append(rep)

    if not all_2021_trades:
        print("❌ 2021 yılında Altın ve Kripto sepetinde işlem oluşmadı.")
        return

    all_2021_trades.sort(key=lambda t: t.entry_date)
    portfolio_report = PerformanceReporter.calculate(
        all_2021_trades,
        name="2021 Gold & Crypto Portfolio",
    ...
    print("\n" + sep)
    print(" 🌟 BEGONYA CONVEXITY │ 2021 YILI ALTIN & KRİPTO ÖZEL BACKTEST KARNESİ 🌟 ")
    print("   • Evren              : Altın (XAUUSD) & Kriptolar (BTCUSD, ETHUSD, SOLUSD)")
    print("   • Dönem              : 01 Ocak 2021 - 31 Aralık 2021 (Tarihi Mega Kripto Boğa Yılı)")
    ...
    # 1. Büyük Sonuç
    print("📊 [BÖLÜM 1] 2021 PORTFÖY GENELİ BÜYÜK SONUÇ")
    print(mid)
    w = 38
    ...
    print("📋 [BÖLÜM 2] ALTIN VE KRİPTOLARIN TEK TEK 2021 KARNESİ")
    print(sep)
    ...
    # 3. Aylık Performans
    print("📅 [BÖLÜM 3] 2021 AYLIK GETİRİ DAĞILIMI (OCAK - ARALIK 2021)")
    print(mid)
    ...
    for m in range(1, 13):
        m_str = f"2021-{m:02d}"
        m_label = f"{m_str}"
        m_trades = [t for t in all_2021_trades if t.exit_date.startswith(m_str)]
        if m_trades:
    ...
    print("🚀 [BÖLÜM 4] 2021 YILININ EN BÜYÜK ASİMETRİK İŞLEMLERİ (TOP RUNNERS)")
    print(mid)
    print(f"{'Giriş':<11} {'Çıkış':<11} {'Varlık':<8} {'Yön':<6} {'Giriş':<11} {'Çıkış':<11} {'Süre(G)':<8} {'Zirve R':<8} {'Net R':<9} {'Net PnL ($)'}")
    print(mid)
    top_winners = sorted(all_2021_trades, key=lambda t: t.pnl_r, reverse=True)[:8]
    ...
    print("📈 [BÖLÜM 5] 2021 ALTIN & KRİPTO BİLEŞİK GETİRİ EĞRİSİ (EQUITY CURVE)")
    print(PerformanceReporter.render_ascii_curve(all_2021_trades, height=10, width=60))
    print(sep + "\n")
    ...
    saved_path = collector.log_backtest_benchmark(
        ...
        notes="2021 Altın & Kripto Özel Yıllık Backtesti (XAUUSD, BTC, ETH, SOL - Mega Boğa)"
    )
    print(f"📁 2021 Benchmark kaydı arşivlendi: {saved_path}\n")

if __name__ == "__main__":
    run_2021_gold_crypto_backtest()
