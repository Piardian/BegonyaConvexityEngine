...
def run_2022_gold_crypto_backtest():
    ...
    all_2022_trades: List[ConvexityTrade] = []
    ...
    for symbol in symbols:
        ...
        # SADECE 2022 yılında açılan işlemler
        trades_2022 = [t for t in full_trades if "2022-01-01" <= t.entry_date <= "2022-12-31"]
        all_2022_trades.extend(trades_2022)

        rep = PerformanceReporter.calculate(trades_2022, name=symbol, initial_capital=initial_capital)
        symbol_reports.append(rep)

    if not all_2022_trades:
        print("❌ 2022 yılında Altın ve Kripto sepetinde işlem oluşmadı.")
        return

    all_2022_trades.sort(key=lambda t: t.entry_date)
    portfolio_report = PerformanceReporter.calculate(
        all_2022_trades,
        name="2022 Gold & Crypto Portfolio",
    ...
    print("\n" + sep)
    print(" 🌟 BEGONYA CONVEXITY │ 2022 YILI ALTIN & KRİPTO ÖZEL BACKTEST KARNESİ 🌟 ")
    print("   • Evren              : Altın (XAUUSD) & Kriptolar (BTCUSD, ETHUSD, SOLUSD)")
    print("   • Dönem              : 01 Ocak 2022 - 31 Aralık 2022 (Büyük Ayı Piyasası & Kripto Kışı)")
    ...
    # 1. Büyük Sonuç
    print("📊 [BÖLÜM 1] 2022 PORTFÖY GENELİ BÜYÜK SONUÇ")
    print(mid)
    w = 38
    ...
    print("📋 [BÖLÜM 2] ALTIN VE KRİPTOLARIN TEK TEK 2022 KARNESİ")
    print(sep)
    ...
    # 3. Aylık Performans
    print("📅 [BÖLÜM 3] 2022 AYLIK GETİRİ DAĞILIMI (OCAK - ARALIK 2022)")
    print(mid)
    ...
    for m in range(1, 13):
        m_str = f"2022-{m:02d}"
        m_label = f"{m_str}"
        m_trades = [t for t in all_2022_trades if t.exit_date.startswith(m_str)]
        if m_trades:
    ...
    print("🚀 [BÖLÜM 4] 2022 YILININ EN BÜYÜK ASİMETRİK İŞLEMLERİ (TOP RUNNERS)")
    print(mid)
    print(f"{'Giriş':<11} {'Çıkış':<11} {'Varlık':<8} {'Yön':<6} {'Giriş':<11} {'Çıkış':<11} {'Süre(G)':<8} {'Zirve R':<8} {'Net R':<9} {'Net PnL ($)'}")
    print(mid)
    top_winners = sorted(all_2022_trades, key=lambda t: t.pnl_r, reverse=True)[:8]
    ...
    print("📈 [BÖLÜM 5] 2022 ALTIN & KRİPTO BİLEŞİK GETİRİ EĞRİSİ (EQUITY CURVE)")
    print(PerformanceReporter.render_ascii_curve(all_2022_trades, height=10, width=60))
    print(sep + "\n")
    ...
    saved_path = collector.log_backtest_benchmark(
        ...
        notes="2022 Altın & Kripto Özel Yıllık Backtesti (XAUUSD, BTC, ETH, SOL - Ayı Piyasası)"
    )
    print(f"📁 2022 Benchmark kaydı arşivlendi: {saved_path}\n")

if __name__ == "__main__":
    run_2022_gold_crypto_backtest()
