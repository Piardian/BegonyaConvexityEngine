...
def run_2024_gold_crypto_backtest():
    cfg_path = PROJECT_DIR / "config" / "convexity_config.json"
    with open(cfg_path, "r", encoding="utf-8") as f:
        config = json.load(f)

    # Sadece Altın ve Kripto Sepeti
    ...
    all_2024_trades: List[ConvexityTrade] = []
    ...
    for symbol in symbols:
        ...
        # SADECE 2024 yılında açılan işlemler
        trades_2024 = [t for t in full_trades if "2024-01-01" <= t.entry_date <= "2024-12-31"]
        all_2024_trades.extend(trades_2024)

        rep = PerformanceReporter.calculate(trades_2024, name=symbol, initial_capital=initial_capital)
        symbol_reports.append(rep)

    if not all_2024_trades:
        print("❌ 2024 yılında Altın ve Kripto sepetinde işlem oluşmadı.")
        return

    all_2024_trades.sort(key=lambda t: t.entry_date)
    portfolio_report = PerformanceReporter.calculate(
        all_2024_trades,
        name="2024 Gold & Crypto Portfolio",
    ...
    print("\n" + sep)
    print(" 🌟 BEGONYA CONVEXITY │ 2024 YILI ALTIN & KRİPTO ÖZEL BACKTEST KARNESİ 🌟 ")
    print("   • Evren              : Altın (XAUUSD) & Kriptolar (BTCUSD, ETHUSD, SOLUSD)")
    print("   • Dönem              : 01 Ocak 2024 - 31 Aralık 2024 (1 Tam Yıl)")
    ...
    # 1. Büyük Sonuç
    print("📊 [BÖLÜM 1] 2024 PORTFÖY GENELİ BÜYÜK SONUÇ")
    print(mid)
    w = 38
    ...
    print("📋 [BÖLÜM 2] ALTIN VE KRİPTOLARIN TEK TEK 2024 KARNESİ")
    print(sep)
    ...
    # 3. Aylık Performans
    print("📅 [BÖLÜM 3] 2024 AYLIK GETİRİ DAĞILIMI (OCAK - ARALIK 2024)")
    print(mid)
    ...
    for m in range(1, 13):
        m_str = f"2024-{m:02d}"
        m_label = f"{m_str}"
        m_trades = [t for t in all_2024_trades if t.exit_date.startswith(m_str)]
        if m_trades:
    ...
    print("🚀 [BÖLÜM 4] 2024 YILININ EN BÜYÜK ASİMETRİK İŞLEMLERİ (TOP RUNNERS)")
    print(mid)
    print(f"{'Giriş':<11} {'Çıkış':<11} {'Varlık':<8} {'Yön':<6} {'Giriş':<11} {'Çıkış':<11} {'Süre(G)':<8} {'Zirve R':<8} {'Net R':<9} {'Net PnL ($)'}")
    print(mid)
    top_winners = sorted(all_2024_trades, key=lambda t: t.pnl_r, reverse=True)[:8]
    ...
    print("📈 [BÖLÜM 5] 2024 ALTIN & KRİPTO BİLEŞİK GETİRİ EĞRİSİ (EQUITY CURVE)")
    print(PerformanceReporter.render_ascii_curve(all_2024_trades, height=10, width=60))
    print(sep + "\n")
    ...
    saved_path = collector.log_backtest_benchmark(
        ...
        notes="2024 Altın & Kripto Özel Yıllık Backtesti (XAUUSD, BTC, ETH, SOL)"
    )
    print(f"📁 2024 Benchmark kaydı arşivlendi: {saved_path}\n")

if __name__ == "__main__":
    run_2024_gold_crypto_backtest()
